"""
Python file provided by the TA but modified according to needs to process input data
...
def BalanceDataset(features, labels):
 ...
 // balanced_count = min(count_0, count_1)
  ...
  return features[:train_count_0]+features[-train_count_1:],labels[:train_count_0] + labels[-train_count_1:], features[train_count_0:test_count_0]+features[-test_count_1:-train_count_1],labels[train_count_0:test_count_0] + labels[-test_count_1:-train_count_1]
...
def NormalizeFeatures(features,means,variances):
  """
...
def PrintDataToSvmLightFormat(features, labels, filename):
  """  
...
def main():
  ...
  train_features, train_labels, test_features, test_labels = BalanceDataset(features, labels)
  train_features, train_labels = ConvertDataToArrays(train_features, train_labels)
  test_features, test_labels = ConvertDataToArrays(test_features, test_labels)
  ...
  PrintDataToSvmLightFormat(train_features, train_labels,"svm_train_features.data")
  PrintDataToSvmLightFormat(test_features, test_labels,"svm_test_features.data")
